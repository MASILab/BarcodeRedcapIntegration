...
class RedcapLabelwriterApp_v3(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.title = 'CMA PTSD Redcap LabelPrinter Toolkit_v3'
        self.left = 10
        self.top = 10
        self.width = 800 # 400
        self.height = 585 # 585
        ...
        f_redcap_key = open("/app/v3/PrinterApp_v3/REDCAP_API_KEY.txt", "r")
    ...
    def initUI(self):
        ...
        self.cb_study_type.addItems(["CMA_PTSD"])    
        ...
        # specific for CMA PTSD
        self.cb_option_type.addItems(["Default",
                                      "Need extra biopsies from TI"]) 
...
#        self.cb_processed_by.setCurrentIndex(1)
        ...
        pixmap = QPixmap('/app/v3/PrinterApp_v3/masi.png')
...
#         self.btn_gen_new_pack = QPushButton('Generate and Print\nnew Barcode Pack', self)
        ...
        # specific for CMA PTSD 
        self.btn_print_extra_TI_only = QPushButton('Extra TI only', self)
        self.btn_print_extra_TI_only.resize(180,30)
        self.btn_print_extra_TI_only.move(200,245)
...
#         self.btn_print_serum_only.move(260,245)
        ...
        self.cb_action_type.resize(280,30)
        self.cb_action_type.move(80,330)
...
#                                       'Distributed to Vantage'])   
        ...
        self.btn_print_extra_TI_only.clicked.connect(self.printExtraTIOnly)
...
#             return
    ...
    def getNextSubject(self):
        ...
        if len(barcodeSubset) == 0 :
            ...
            for x in barcodeSubsetList:
                printBarcodeSubsetList.append(int(x[4:7])) # GCAxxx -> DoD_xxx -> CMA_xxx
        ...
        # DoD app difference, the record_id in DoD app is DoD_xxx
        # tmp_DoD_id = 'DoD_' + self.getPatentId(next_subject_id) # make three digit id
        tmp_DoD_id = 'CMA_' + self.getPatentId(next_subject_id) # make three digit id
        record = df.loc[df['record_id'] == str(tmp_DoD_id)]

        if len(record) is 0 or None:
            QMessageBox.about(self, "WARNING", "---(1)Please create subject %s form first.\n(2)Please double check the Id in Redcap entry.\nThe Id should be in CMA_xxx where xxx is a three-digit value)" % str(self.subjectId))
            # QMessageBox.about(self, "WARNING", '-(1)Please create subject %s form first.\n(2)Please double check the Id in Redcap entry.\nThe Id should be in numeric format (i.e., 17, 20, 99), \nnot in 3-digit format (i.e., 017, 020, 099)\n(3)In Redcap, please check if you choose a category' % str(tmp_DoD_id))
            return
        ...
        # "CMA_PTSD", "CD_ENDO", "CTL_ENDO", "UC_Surgery","CD_Surgery","CTL_Surgery"
        
        if studyType_redcap == 'bccma_arm_1':
            self.cb_study_type.setCurrentIndex(1)
        # elif studyType_redcap == 'cd_arm_1':
        #     self.cb_study_type.setCurrentIndex(2)
        # elif studyType_redcap == 'control_arm_1':
        #     self.cb_study_type.setCurrentIndex(3)
        # elif studyType_redcap == 'uc_arm_2':
        #     self.cb_study_type.setCurrentIndex(4)
        # elif studyType_redcap == 'cd_arm_2':
        #     self.cb_study_type.setCurrentIndex(5)
        # elif studyType_redcap == 'control_arm_2':
        #     self.cb_study_type.setCurrentIndex(6)
        # elif studyType_redcap == 'cancer_arm_2':
        #     self.cb_study_type.setCurrentIndex(7)
    ...
    def getSubjectType(self):
        ...
        df = pd.DataFrame.from_dict(curSubj)
        print("#####################")
        #record = df.loc[df['record_id'] == str(int(self.subjectId))]
        # record = df.loc[df['record_id'] == str('DoD_%s' % self.subjectId)]
        record = df.loc[df['record_id'] == str('CMA_%s' % self.subjectId)]
...
#            last_print_subj = record_list[len(record_list)-1][3:6]
            ...
            last_print_subj = last_record_list[2][4:7] # index 2 is 'barcode_sample_id', sample_id is now DoD_xxx -> CMA_xxx
    ...
    def printExtraTIOnly(self):
        ...
        #"CMA_PTSD"
        if studyType_redcap == 'bccma_arm_1':
            studyType_checker = 'CMA_PTSD'

        #"UC_ENDO", "CD_ENDO", "CTL_ENDO", "UC_Surgery","CD_Surgery","CTL_Surgery"
        # if studyType_redcap == 'uc_arm_1':
        #     studyType_checker = 'UC_ENDO'
        # elif studyType_redcap == 'cd_arm_1':
        #     studyType_checker = 'CD_ENDO'
        # elif studyType_redcap == 'control_arm_1':
        #     studyType_checker = 'CTL_ENDO'
        # elif studyType_redcap == 'uc_arm_2':
        #     studyType_checker = 'UC_Surgery'
        # elif studyType_redcap == 'cd_arm_2':
        #     studyType_checker = 'CD_Surgery'
        # elif studyType_redcap == 'control_arm_2':
        #     studyType_checker = 'CTL_Surgery'
        # elif studyType_redcap == 'cancer_arm_2':
        #     studyType_checker = 'Cancer_Surgery'    
...
#             return
        ...
        print('NIMANIMANIMANIMA %s' % str(nextAvailActionId))
        ...
        tmp_barcode_list = barcodeExec.get_extra_TI_barcode_list()
        
#         if len(tmp_barcode_list) is not 4: # only 4 extra barcode ids. -> false, CTL_ENDO has 2, CD_Surgery has 2, CTL_Surgery has 8.... decide to remove the condition. 
...
#             return
        ...
        for tmp_barcode_id in tmp_barcode_list:
            ...
            # CMA we don't need to update because the printing extra TI won't matter on the main redcap form.
    ...
    def genPrintNewPack(self):
        ...
        #DoD id is DoD_xxx instead of xxx
        #record = df.loc[df['record_id'] == str(int(self.subjectId))]
        record = df.loc[df['record_id'] == str('CMA_%s' % self.subjectId)]
...
# we use DoD ID: DoD_xxx
            bool_update_redcap_sophie = self.redcapExec.update_REDCAP_SOPHIE(self.studyType,'CMA_%s' %self.subjectId,tmp_barcode_list,self.custom_option)
        ...
        self.plain_console.appendPlainText("\n\n%s\nGenerate Extra TI Barcodes IDs for subject %s DONE! Printing may takes time, please be patient.\nTotal barcodes to print: %s" 
                                           % (str(cur_date),str(self.subjectId),str(len(tmp_barcode_list))))
...
#             return
        ...
        self.subjectId = self.barcode_id[4:7] #GCAxxx -> xxx -> DoD_xxx -> xxx
...
#             return
        ...
        nextAvailActionId = self.redcapExec.getNextAvailActionId()
        self.subjectId = self.barcode_id[4:7] # DoD_xxx -> xxx
        ...
        if self.action_type == 'printed - Re-print a destroyed barcode':
            self.rePrintSingleLabel(ean8_code)
            bool_update_redcap_sophie = self.redcapExec.update_REDCAP_SOPHIE_SINGLE_BARCODE(self.studyType,str(int(self.subjectId)),self.barcode_id,tmp_custom_option)  
