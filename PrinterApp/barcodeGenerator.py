...
class BarcodeGenerator:
    def __init__(self, p_id=None, sType=None):
        self._subject_id = p_id # three digit subject id
        self._studyType =  sType # selected study type
        self._barcode_list = [] # a array list to store all barcode IDs to print. 

    def execute(self):
        """
        generate all BarcodeId according to study type
        """
        
        # stool, Serum and DNA collection are only available for CD_ENDO and CTL_ENDO
    ...
    """
    generate all biopsies ids - CD_ENDO 
    """ 
    ...
    """
    generate all biopsies ids - CTL_ENDO 
    """           
    ...
    """
    generate all biopsies ids - CD_Surgery
    """          
        self._barcode_list.append('GCA%sFreshTI' % self._subject_id)
        self._barcode_list.append('GCA%sFixedTI' % self._subject_id)
        self._barcode_list.append('GCA%sFrozenTI' % self._subject_id)
    ...
    """
    generate all biopsies ids - CTL_Surgery
    """         
    ...
    """
    Extra frozen biopsies ids for CTL_ENDO, CD_Surgery, and CTL_Surgery, when fresh biopsies is not available
    """"
        if self._studyType == 'CD_ENDO':
            ...
            self._barcode_list.append('GCA%sADDFrozenTI' % self._subject_id)
            self._barcode_list.append('GCA%sADDFrozenAC' % self._subject_id)
        elif self._studyType == 'CD_Surgery':
            self._barcode_list.append('GCA%sADDFrozenTI' % self._subject_id)
            self._barcode_list.append('GCA%sADDFrozenAC' % self._subject_id)
        elif self._studyType == 'CTL_Surgery':
            self._barcode_list.append('GCA%sADDFrozenTI1' % self._subject_id)
            self._barcode_list.append('GCA%sADDFrozenTI2' % self._subject_id)
            self._barcode_list.append('GCA%sADDFrozenTI3' % self._subject_id)
            self._barcode_list.append('GCA%sADDFrozenAC4' % self._subject_id)
            self._barcode_list.append('GCA%sADDFrozenAC5' % self._subject_id)
            self._barcode_list.append('GCA%sADDFrozenAC6' % self._subject_id)
            self._barcode_list.append('GCA%sADDFrozenAC7' % self._subject_id)
            self._barcode_list.append('GCA%sADDFrozenAC8' % self._subject_id)
            
    def get_extra_frozen_barcode_list(self):
    """
    get ADD frozen id only
    """
        self._barcode_list = []
    ...
    """
    get barcode id list according to the custom option
    """
    ...
    def ean8_to_recordId(self,tmp_ean8_code):
        """
        record id is GCAxxxSpecimenType, ean8 code is the hacking barcode to generate
        """
    ...
    def recordId_to_ean8(self,tmp_barcode_id):
        """
        record id is GCAxxxSpecimenType, ean8 code is the barcode to generate
        """
        eanConv = CheatSheetEAN_Converter()
        #"CD_ENDO", "CTL_ENDO", "CD_Surgery","CTL_Surgery"
        ean8_code = ''# type(1 digit)_cheatList(2 digit)_0_patientid(3 digit)_checksum(1 digit)
    ...
    """
    Depreciate. We used to collect 8 Serum only. 
    """   
