...
class RedcapHelper:
    """
    Class for all redcap field import / export operations. 
    """
    ...
    def getBarcodeSubsetAndCustomOption(self):
        """
        Get all records in action_tuple_table_arm_3's barcode sample id and custom option fields.
        """
    ...
    def getSubjectLastestCustomOption(self, barcode_id,subject_id):
        """
        fyi, 
        (1) check if barcode exist,
        (2) if barcode exist, get custom option
        (3) option may change from "Default" to "Need Extra Frozen" if the subject has already been printed via default option.
        """    
